...
class GE_MB_TrabajoActualidad extends GE_Metabox{
    
    public static function add($post_type_key){
        $post_types = [$post_type_key]; //ge_funcionarios

        add_meta_box(
            'ge_funcionarios_trabajo_actualidad',
    ...
    public static function html($post, $metabox){
        ...
        $contenido = get_post_meta($post->ID, '_ge_funcionario_ta', true);
        ...
        wp_editor($contenido_value, "id_wpe_trabajo_actualidad", $settings );
    }
    ...
    public static function save($post_id, $post, $update){
        ...
        if(array_key_exists('id_wpe_trabajo_actualidad', $_POST)){
            global $allowedposttags;
          $sanitizado = wp_kses( $_POST['id_wpe_trabajo_actualidad'], $allowedposttags );    
